    ...
    def build_network(self):
        ...
        with tf.variable_scope('VDSR'):
            self.weights_t = {
                ...
                'w{:d}'.format(self.layer_depth): tf.get_variable('w{:d}'.format(self.layer_depth), [3, 3, 64, 1], initializer=initializer_w, dtpye=tf.float32)
            }

            self.biases_t = {
                'b1': tf.get_variable('b1', [64], initializer=initializer_b, dtpye=tf.float32),
                'b{:d}'.format(self.layer_depth): tf.get_variable('w{:d}'.format(self.layer_depth), [1], initializer=initializer_b, dtpye=tf.float32)
            }

            for i in range(2, self.layer_depth):
                self.weights_t['w{:d}'.format(i)] = tf.get_variable('w{:d}'.format(i), [3, 3, 64, 64], initializer=initializer_w, dtpye=tf.float32)
                self.biases_t['b{:d}'.format(i)] = tf.get_variable('b{:d}'.format(i), [64], initializer=initializer_b, dtpye=tf.float32)
